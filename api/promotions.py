...
def create_promotion(data: PromotionBase):
    ...
    try:
        cur.execute("""
            INSERT INTO promotions(name, discount_pct, is_active, type, code, target_margin)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (data.name.strip(), data.discount_pct, data.is_active, data.type, data.code, data.target_margin))
...
def update_promotion(promo_id: int, data: PromotionBase):
    ...
    try:
        cur.execute("""
            UPDATE promotions 
            SET name=%s, discount_pct=%s, is_active=%s, type=%s, code=%s, target_margin=%s 
            WHERE id=%s
        """, (data.name, data.discount_pct, data.is_active, data.type, data.code, data.target_margin, promo_id))
