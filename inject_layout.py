...
end_marker = 'function TabButton'


start_idx = content_normalized.find(start_marker)
end_idx = content_normalized.find(end_marker)

if start_idx == -1 or end_idx == -1:
    print(f"Error finding markers. start: {start_idx}, end: {end_idx}")
else:
    pre = content_normalized[:start_idx]
    post = content_normalized[end_idx:]
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    {status === 'COTIZACION' && promotions.length > 0 && (
                        <section className="bg-premium-slate/50 p-6 rounded-3xl border border-white/5 space-y-3 shadow-xl ring-1 ring-white/5">
                    ...
                    <section className="bg-premium-slate/60 p-6 rounded-3xl border border-white/5 space-y-4 shadow-xl ring-1 ring-white/5">
...
\n\n"""
    ...
    final_content = pre + new_return_jsx + post

    with codecs.open('frontend/src/components/Sales.jsx', 'w', encoding='utf-8') as fOut:
        fOut.write(final_content)
    print("Injected correctly with normalized lines.")
